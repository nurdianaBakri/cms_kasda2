...
class PemeliharaanKasda extends CI_Controller {
	...
	{   
        ...
        if($jenis_aksi=="add")
        {
            ...
            //Check if kasdanya sama
            $this->db->where('KD_KASDA',$data['KD_KASDA']);
            $Check = $this->db->get('ref_kasda');
            if ($Check->num_rows()>0)
            {
                $hasil['status']=3;
            }
        ...
        if($hasil['status']==1 && $jenis_aksi=="add")
        {
            $hasil['pesan'] ="Proses Tambah data Kasda berhasil";
        }
        else if($hasil['status']==3 && $jenis_aksi=="add"){
            $hasil['pesan'] ="Proses Tambah data Kasda gagal, karena KODE KASDA sudah digunakan. Silahkan masukkan KODE KASDA yang lain";
        }
        else if ($hasil['status']==0 && $jenis_aksi=="add")
        {
            $hasil['pesan'] ="Proses Tambah data Kasda gagal"; 
        }
        else if ($hasil['status']==1 && $jenis_aksi=="edit")
        {
