...
class LapPembukaansp2d extends CI_Controller {
	...
	{ 
        ...
        $data['url'] = "laporan/LapTrsans/LapPembukaansp2d/";    
        //kd kasda 
        $data['kasda']= $this->M_laporan_rekKoran->get_kasda();
 
		$this->load->view('include/header'); 
        $this->load->view('laporan/transaksi/pembukaan_sp2d/index',$data);
    ...
    {    
        ...
        $data['judul_tabel'] = "Rekap Pembukaan SP2D";
        $data['judul_sub_tabel'] = "Periode ".$data['tglawal']." s/d ".$data['tglakhir'];
        if ($data['is_empty']==true)
        {
            $this->load->view('laporan/transaksi/pembukaan_sp2d/data_is_empty',$data); 
        }
        else
        { 
            $this->load->view('laporan/transaksi/pembukaan_sp2d/data',$data); 
        }
    }
    ...
    { 
        ...
        $cek = $this->M_laporan_SP2D_sudah_cair->rekap_pembukaan($KD_KASDA, $tglawal, $tglakhir, $user_id);
        if ($cek->num_rows()>0)
        {
            $data['is_empty'] = false; 
    ...
    {   
        $data = $this->get_post_data();
        $this->load->view('laporan/transaksi/pembukaan_sp2d/print_view',$data);  
    }   
    ...
    function get_user_id()
    {
        ...
        $cek = $this->M_laporan_SP2D_sudah_cair->get_user_pembuka_sp2d($KD_KASDA);
        if($cek->num_rows()>0)
        {
        ...
        $this->load->view('laporan/transaksi/pembukaan_sp2d/user_pencairan',$data);  
    }
    ...
    { 
        ...
        $excel->getProperties()->setCreator('My Notes Code')
                        ...
                        ->setTitle("Rekap Pembukaan SP2D")
                        ->setSubject("Siswa")
                        ->setDescription("Rekap Pembukaan SP2D")
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', "Rekap Pembukaan SP2D"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        foreach($data2['laporan'] as $key2){   
            ...
            $timestamp = strtotime($key2['tgl_input']); 
        ...
        header('Content-Disposition: attachment; filename="LAPORAN PEMBUKAAN SP2D.xlsx"'); 
